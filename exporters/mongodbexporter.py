...
def _jsonizer(obj, specify_object_type=True):
...
class _MongoImportFileSet(object):
    ...
    def __init__(self, base_path):
        ...
        self.connection = _MongoImportFileSetConnectionMock(
            *(self._artists, self._labels, self._releases, self._masters))
...
class MongoDbExporter(object):
    ...
    def finish(self, completely_done=False):
        ...
        if 'releases' in collections:
            ...
            self.db.releases.ensure_index([('l_artist', pymongo.ASCENDING), ('l_title', pymongo.ASCENDING)],
                                          background=True)
